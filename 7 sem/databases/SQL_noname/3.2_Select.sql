...
-- 2. Найти имена сотрудников, работавших или работающих в тех же отделах, в которых работал или
...
-- 3. Найти имена сотрудников, работавших или работающих в тех же отделах и должностях, что и сотрудник
-- 'RICHARD MARTIN'.
SELECT EMPNAME FROM EMP
    WHERE EMPNO IN (SELECT DISTINCT EMPNO FROM CAREER
        WHERE DEPTNO IN (SELECT DEPTNO FROM CAREER
...
-- 4. Найти сведения о номерах сотрудников, получивших за какой-либо месяц зарплату большую чем средняя
-- зарплата   за 2000 г. или большую чем средняя зарплата за 2001г.
SELECT EMPNO FROM SALARY
    WHERE SALVALUE > ANY(SELECT AVG(SALVALUE) FROM SALARY WHERE YEAR BETWEEN 2000 AND 2001)

-- 5. Найти сведения о номерах сотрудников, получивших зарплату за какой-либо месяц большую чем среднии зарплаты за все
-- годы начислений.
SELECT EMPNO FROM SALARY
...
-- 9. Определить номера отделов, в которых работали или работают сотрудники, не имеющие начисления зарплаты
SELECT DEPTNO
    FROM DEPT D
    WHERE NOT EXISTS (
    SELECT SALVALUE
...
-- 12. Разделите сотрудников на возрастные группы: A) возраст 20-30 лет; B) 31-40 лет; C) 41-50; D) 51-60 или возраст не
-- определён.
